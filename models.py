...
def base_swin(model_path, num_classes, image_size, pretrained=True,cond=True, where=0):
    model = timm.create_model(model_path, num_classes=num_classes, pretrained=pretrained)
    for n, p in model.named_parameters():
      if p.requires_grad:
        p.requires_grad=False
        # print(p.requires_grad)
    if cond: return InterSwin(model, image_size, num_classes, where=where)
...
def base_resnet101(model_path, num_classes, image_size, pretrained=True, cond=True, where=0):
    # model = models.resnet101(pretrained=pretrained)
    model = timm.create_model(model_path, num_classes=num_classes, pretrained=pretrained)
    for n, p in model.named_parameters():
      if p.requires_grad:
        p.requires_grad=False
    if cond: return InterResnetV2(model, image_size, num_classes, where)
